...
void initLacInterprete(int * symbolTable, int * VM, int * posSymbol, int * posVM) {
    ...
    // Adding functions allowed in interprete mode
...
int main(int argc, char * argv[]) { // it should not take any input
    ...
    linkProcessor(&data, &type, NULL, &stringMem[0], VM, &posMemory);
    ...
    while(1) {
        ...
        do {
            if(lexemeList[posLexeme].type == C) {
                ...
                cstrncpyMEM(&stringMem[0], &lineBuffer[lexemeList[posLexeme].begin], &posMemory, length);
            } else {
                // It is an identifier
